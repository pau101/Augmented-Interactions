...
public final class AnimationBottleDrain extends AnimationBottle {
	public AnimationBottleDrain(ItemStack stack, int actionBarSlot, EnumHand hand, RayTraceResult mouseOver, Predicate<ItemStack> itemPredicate, float fluidLevel) {
	...
	public void transform(MatrixStack matrix, Minecraft mc, World world, EntityPlayer player, float yaw, boolean isLeft, float delta) {
		...
		matrix.translate(pos.getX() + 0.5, pos.getY() + getFluidLevel() + sin * 1.5 * (2 - percent) + 0.1, pos.getZ() + 0.5);
